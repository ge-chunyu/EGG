function analyzeEGG()
    ...
    data = table([speaker], ["a"],[0],[0],[0],[0],[0],[0],[0],[0],[0],...
                 'VariableNames',{'filename', 'label', 'normTime',...
                 'point', 'nFoc', 'nFov', 'nCqc', 'nSqc', 'nCqh', 'nPic', 'nSqh'});
    ...
    for i = 1:fileNum
        ...
        if startTime ~= 0
            ...
            if length(peakidx) >= 13
                [cycle, dEGGpks, fo_c, fo_v, cq_c, sq_c, cq_h, sq_h] = calcPara(Fs, dpeakidx, maxContact, opening, noContact, 4);
                [normTime, nFoc, nFov, nCqc, nSqc, nCqh, nPic, nSqh] = normPara(startTime, endTime, nPoint, cycle, fo_c, fo_v, cq_c, sq_c, cq_h, pic, sq_h);
                filename = repmat(files(i).name, 12, 1);
                label = repmat(rhyme, 12, 1);
                point = [1:12]';
                normTime = round(normTime', 2);
                temp = table(filename, label, normTime, point, nFoc, nFov, nCqc, nSqc, nCqh, nPic, nSqh);
